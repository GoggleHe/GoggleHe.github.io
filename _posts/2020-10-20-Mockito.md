...
- `ArgumentMatcher<T>`接口为参数匹配器接口，实现其实现类配合`Mockito.argThat()`方法可自定义参数匹配器
...
- mock待测试方法行为
- 未mock的参数输入，若返回值为基本类型，返回默认值，若为class类返回null
...
- 解决链式调用必须一层一层mock所有对象及其调用的方法的问题
